    ...
    <div className="mx-auto max-w-4xl p-6 lg:p-10">
      <div className="mb-10">
        ...
        <div className="mt-6 flex items-center gap-2 text-sm font-medium">
          <span className={`px-3 py-1 rounded-full ${step >= 1 ? "bg-accent text-white" : "bg-elevated text-muted"}`}>1. Upload</span>
          <span className="text-line">-</span>
          <span className={`px-3 py-1 rounded-full ${step >= 2 ? "bg-accent text-white" : "bg-elevated text-muted"}`}>2. Preview</span>
          <span className="text-line">-</span>
          <span className={`px-3 py-1 rounded-full ${step >= 3 ? "bg-accent text-white" : "bg-elevated text-muted"}`}>3. Mapping</span>
          <span className="text-line">-</span>
          <span className={`px-3 py-1 rounded-full ${step >= 4 ? "bg-accent text-white" : "bg-elevated text-muted"}`}>4. World</span>
        </div>
      </div>

      {step === 1 && (
        <div className="rounded-2xl border border-line bg-surface p-8 text-center">
          <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-elevated">
            <UploadCloud size={40} className="text-accent" />
          </div>
          <h2 className="text-2xl font-bold text-white">Upload HTML Export</h2>
          <p className="mt-2 text-muted">Export your Telegram group history as HTML and upload the `messages.html` file here.</p>
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-6">
            <input 
              ...
              className="block w-full max-w-sm text-muted file:mr-4 file:rounded-full file:border-0 file:bg-accent file:px-6 file:py-3 file:text-sm file:font-semibold file:text-white hover:file:bg-accent-soft" 
            />
            
            <button 
              onClick={parseFile} 
              disabled={!file || parsing}
              className="flex items-center justify-center gap-2 rounded-full bg-accent px-8 py-4 text-base font-bold text-white shadow-lg hover:bg-accent-soft disabled:opacity-50"
            >
      ...
      {step === 2 && (
        <div className="rounded-2xl border border-line bg-surface p-8">
          <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-elevated">
            <Users size={32} className="text-accent" />
          </div>
          <h2 className="text-2xl font-bold text-white">Import Preview</h2>
          <p className="mt-2 text-muted">We&rsquo;ve successfully read your file. Here is what we found:</p>
          ...
          <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <div className="rounded-xl bg-canvas p-4 border border-line col-span-2">
          ...
          <button 
            onClick={() => setStep(3)}
            className="mt-8 flex w-full items-center justify-center gap-2 rounded-full bg-accent px-8 py-4 text-base font-bold text-white shadow-lg hover:bg-accent-soft"
          >
      ...
      {step === 3 && (
        <div className="space-y-6">
          <div className="rounded-2xl border border-line bg-surface p-8">
            ...
            <p className="text-muted mb-8">Link each Telegram participant to a character in RPWeb, or create a new one instantly.</p>
            
            <div className="space-y-6">
              {participants.map(p => (
                <div key={p} className="rounded-xl border border-line bg-canvas p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                    <div className="w-full sm:w-1/3">
                      <div className="text-sm text-muted">Telegram User</div>
                    ...
                    <div className="w-full sm:w-2/3">
                      <select 
                        ...
                        className="block w-full rounded-lg border border-line bg-elevated px-4 py-3 text-white focus:border-accent focus:ring-1 focus:ring-accent"
                      >
                  ...
                  {characterMap[p] === "CREATE_NEW" && (
                    <div className="mt-4 border-t border-line pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-4 duration-300">
                      <div>
                        ...
                        <input 
                          ...
                          className="mt-1 block w-full rounded-md border border-line bg-surface px-3 py-2 text-white text-sm"
                        />
                      </div>
                      <div>
                        <label className="text-xs font-medium text-muted">Avatar URL (Optional)</label>
                        <input 
                          type="text" 
                          placeholder="https://..."
      ...
      {step === 4 && (
        <div className="rounded-2xl border border-line bg-surface p-8">
          ...
          <p className="text-muted mb-8">Set up the world where this imported history will live.</p>
          ...
          <div className="space-y-6">
            <div>
              ...
              <input 
                ...
                className="mt-2 block w-full rounded-lg border border-line bg-canvas px-4 py-3 text-white focus:border-accent focus:ring-1 focus:ring-accent" 
              />
            </div>
            ...
            <div>
              <label className="text-sm font-medium text-muted">Join World As (Your Character)</label>
              <select 
                value={myCharacterId} 
